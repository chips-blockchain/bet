...
int32_t BET_rest_dcv_init(struct lws *wsi, cJSON *argjson) {
    ...
    if (BET_dcv == NULL)
        BET_dcv = (struct privatebet_info *)malloc(
            sizeof(struct privatebet_info)); //   calloc(1,sizeof(struct
                                             //   privatebet_info));
    else
        memset(BET_dcv, 0x00, sizeof(struct privatebet_info));

    if (DCV_VARS == NULL)
        DCV_VARS = (struct privatebet_vars *)malloc(
            sizeof(struct privatebet_vars)); //	DCV_VARS =
...
int32_t BET_rest_dcv_process_init_p(struct lws *wsi, cJSON *argjson) {
    ...
    retval = sg777_deckgen_vendor(
        peerid, dcv_info.cardprods[peerid], dcv_info.dcvblindcards[peerid],
        BET_dcv->range, cardpubvalues, dcv_info.deckid);
...
int32_t BET_rest_dcv_deck_init_info(struct lws *wsi, cJSON *argjson) {
    ...
    for (int i = 0; i < dcv_info.numplayers; i++) {
        for (int j = 0; j < BET_dcv->range; j++) {
            cJSON_AddItemToArray(cjsondcvblindcards,
    ...
    for (int i = 0; i < dcv_info.numplayers; i++) {
        for (int j = 0; j < BET_dcv->range; j++) {
            cJSON_AddItemToArray(cjsong_hash, cJSON_CreateString(bits256_str(
                                                  str, g_hash[i][j])));
        }
    }
...
int32_t BET_rest_evaluate_hand(struct lws *wsi) {
    ...
    for (int i = 0; i < dcv_maxplayers; i++) {
        if (winners[i] == 1) {
            BET_rest_DCV_create_invoice_request(
                wsi, (DCV_VARS->pot / no_of_winners), i);
        }
    }
    printf("\n");
...
int32_t BET_rest_create_invoice(struct lws *wsi, cJSON *argjson) {
    ...
    if (jint(invoice, "code") != 0) {
        ...
        if (strcmp(
                jstr(cJSON_GetObjectItem(argjson, "payment_params"), "action"),
                "round_betting") == 0) {
            cJSON_AddNumberToObject(invoiceInfo, "option",
                                    jint(argjson, "option"));
        }
        ...
        lws_write(wsi, cJSON_Print(invoiceInfo),
                  strlen(cJSON_Print(invoiceInfo)), 0);
    }
            ...
            for (int j = 0; j < bet->maxplayers; j++) {
                if (card_matrix[j][i] == 0) {
                    retval = BET_send_turn_info(
                        bet, j, (i * bet->maxplayers) + j, hole_card);
                    goto end;
                }
            }
        }
...
int32_t BET_settle_game(cJSON *payInfo, struct privatebet_info *bet,
    ...
    if (jint(invoicesInfo, "code") != 0) {
        ...
        if (no_of_cards == no_of_bets) {
            ...
            cJSON_AddNumberToObject(winnerInfo, "winning_amount",
                                    dcv_info.betamount);
    ...
    if (players_left < 2) {
        ...
        for (int i = 0; i < bet->maxplayers; i++) {
            if (winners[i] == 1) {
                ...
                retval = BET_DCV_invoice_pay(bet, vars, i,
