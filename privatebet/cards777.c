...
int32_t BET_ciphercreate(bits256 privkey, bits256 destpub, uint8_t *cipher,
    ...
    buffer =
        malloc(datalen + crypto_box_NONCEBYTES + crypto_box_ZEROBYTES + 256);
    msglen =
        _SuperNET_cipher(nonce, ptr, data, msglen, destpub, privkey, buffer);
    crc32 = calc_crc32(0, ptr, msglen);
    msglen += crypto_box_NONCEBYTES;
    if ((0)) {
        char str[65];
        fprintf(stderr, "encode into msglen.%d crc32.%u [%llx] %s\n", msglen,
                crc32, *(long long *)cipher,
...
uint8_t *BET_decrypt(uint8_t *decoded, int32_t maxsize, bits256 senderpub,
    ...
    if ((0)) {
        ...
        printf(" decrypt [%llx] recvlen.%d crc32.%u %s\n", *(long long *)ptr,
               recvlen, calc_crc32(0, cipher, cipherlen),
               bits256_str(str, curve25519(mypriv, senderpub)));
    }
    if (cipherlen > 0 && cipherlen <= maxsize) {
        if ((dest = _SuperNET_decipher(nonce, cipher, decoded, cipherlen,
                                       senderpub, mypriv)) != 0) {
