...
int32_t BET_rest_dcv_init(struct lws *wsi, cJSON *argjson) {
  ...
  if (BET_dcv == NULL)
    BET_dcv = (struct privatebet_info *)malloc(sizeof(
        struct privatebet_info)); //   calloc(1,sizeof(struct privatebet_info));
  else
    memset(BET_dcv, 0x00, sizeof(struct privatebet_info));

  if (DCV_VARS == NULL)
    DCV_VARS = (struct privatebet_vars *)malloc(sizeof(
        struct privatebet_vars)); //	DCV_VARS = calloc(1,sizeof(struct
                                  // privatebet_vars));
  else
...
int32_t BET_rest_default(struct lws *wsi, cJSON *argjson) {
  ...
  cJSON_AddStringToObject(defaultInfo, "default",
                          "No method found in the back end for the action");
...
int32_t BET_rest_client_join_req(struct lws *wsi, cJSON *argjson) {
  ...
  cJSON_AddNumberToObject(playerInfo, "peerid",
                          BET_dcv->numplayers -
                              1); // players numbering starts from 0(zero)
...
int32_t BET_rest_broadcast_table_info(struct lws *wsi) {
  ...
  cJSON_AddItemToObject(tableInfo, "playersInfo",
                        playersInfo = cJSON_CreateArray());
  for (int32_t i = 0; i < BET_dcv->maxplayers; i++) {
    cJSON_AddItemToArray(playersInfo, cJSON_CreateString(bits256_str(
...
int32_t BET_rest_dcv_process_init_p(struct lws *wsi, cJSON *argjson) {
  ...
  retval = sg777_deckgen_vendor(peerid, dcv_info.cardprods[peerid],
                                dcv_info.dcvblindcards[peerid], BET_dcv->range,
...
int32_t BET_rest_dcv_deck_init_info(struct lws *wsi, cJSON *argjson) {
  ...
  for (int i = 0; i < dcv_info.numplayers; i++) {
    for (int j = 0; j < BET_dcv->range; j++) {
      cJSON_AddItemToArray(cjsoncardprods, cJSON_CreateString(bits256_str(
  ...
  for (int i = 0; i < dcv_info.numplayers; i++) {
    for (int j = 0; j < BET_dcv->range; j++) {
      cJSON_AddItemToArray(cjsong_hash,
                           cJSON_CreateString(bits256_str(str, g_hash[i][j])));
    }
  }
  cJSON_AddItemToObject(deck_init_info, "peerpubkeys",
                        cjsonpeerpubkeys = cJSON_CreateArray());
  for (int i = 0; i < dcv_info.numplayers; i++) {
    cJSON_AddItemToArray(cjsonpeerpubkeys, cJSON_CreateString(bits256_str(
                                               str, dcv_info.peerpubkeys[i])));
  }

  // printf("\n%s:%d::%s",__FUNCTION__,__LINE__,cJSON_Print(deck_init_info));
  lws_write(wsi, cJSON_Print(deck_init_info),
            strlen(cJSON_Print(deck_init_info)), 0);
  return retval;
}
...
int32_t BET_rest_send_turn_info(struct lws *wsi, int32_t playerid,
    ...
    for (int i = 0; i < no_of_hole_cards; i++) {
      for (int j = 0; j < BET_dcv->maxplayers; j++) {
        if (card_matrix[j][i] == 0) {
          retval = BET_rest_send_turn_info(
              wsi, j, (i * (BET_dcv->maxplayers)) + j, hole_card);
          goto end;
        }
      }
    }
  } else if (flop_cards_drawn == 0) {
    for (int i = no_of_hole_cards; i < no_of_hole_cards + no_of_flop_cards;
         i++) {
      for (int j = 0; j < BET_dcv->maxplayers; j++) {
        if (card_matrix[j][i] == 0) {
          if ((i - (no_of_hole_cards)) == 0) {
            retval = BET_rest_send_turn_info(
                wsi, j,
                (no_of_hole_cards * (BET_dcv->maxplayers)) +
    ...
    for (int i = no_of_hole_cards + no_of_flop_cards;
      ...
      for (int j = 0; j < 2 /*BET_dcv->maxplayers*/; j++) {
        if (card_matrix[j][i] == 0) {
          retval = BET_rest_send_turn_info(
              wsi, j,
              (no_of_hole_cards * 2 /*BET_dcv->maxplayers*/) +
...
int32_t BET_rest_receive_card(struct lws *wsi, cJSON *playerCardInfo) {
  ...
  if (card_type == hole_card) {
    card_matrix[(cardid % BET_dcv->maxplayers)]
               [(cardid / BET_dcv->maxplayers)] = 1;
    card_values[(cardid % BET_dcv->maxplayers)]
               [(cardid / BET_dcv->maxplayers)] =
    ...
    card_matrix[playerid]
               [no_of_hole_cards + no_of_flop_cards + no_of_turn_card] = 1;
    card_values[playerid]
  ...
  if (hole_cards_drawn == 0) {
    ...
    for (int i = no_of_hole_cards;
         ((i < no_of_hole_cards + no_of_flop_cards) && (flag)); i++) {
      for (int j = 0; ((j < BET_dcv->maxplayers) && (flag)); j++) {
        if (card_matrix[j][i] == 0) {
          flag = 0;
        }
...
int32_t BET_rest_evaluate_hand(struct lws *wsi) {
  ...
  if (players_left < 2) {
    if (only_winner != -1) {
      printf("\nWinning player is :%d, winning amount:%d", only_winner,
             DCV_VARS->pot);
      goto end;
    }
  }
  ...
  for (int i = 0; i < dcv_maxplayers; i++) {
    if (winners[i] == 1) {
      BET_rest_DCV_create_invoice_request(wsi, (DCV_VARS->pot / no_of_winners),
                                          i);
    }
  }
  printf("\n");
  ...
  for (int i = 0; i < BET_dcv->maxplayers; i++) {
    ...
    while ((channel_state = LN_get_channel_status(channel_id)) != 3) {
      if (channel_state == 2) {
        ...
        retval = -1;
        printf(
            "\n%s:%d: Player: %d is failed to establish the channel with DCV",
...
int32_t BET_rest_create_invoice(struct lws *wsi, cJSON *argjson) {
  ...
  sprintf(argv[4], "Invoice_details_playerID:%d,round:%d,betting_Amount:%d",
  ...
  if (jint(invoice, "code") != 0) {
    ...
    cJSON_AddNumberToObject(invoiceInfo, "betAmount",
                            jint(argjson, "betAmount"));

    if (strcmp(jstr(cJSON_GetObjectItem(argjson, "payment_params"), "action"),
               "round_betting") == 0) {
    ...
    cJSON_AddItemToObject(invoiceInfo, "payment_params",
                          cJSON_GetObjectItem(argjson, "payment_params"));
    lws_write(wsi, cJSON_Print(invoiceInfo), strlen(cJSON_Print(invoiceInfo)),
              0);
  }
...
int32_t BET_rest_DCV_create_invoice(struct lws *wsi, cJSON *argjson) {
  ...
  sprintf(argv[3], "%d_%d_%s", jint(argjson, "playerID"),
          jint(argjson, "winningAmount"),
          bits256_str(hexstr, BET_get_deckid(jint(argjson, "playerID"))));
  sprintf(argv[4], "\"Invoice details playerID:%d,winning Amount:%d\"",
          jint(argjson, "playerID"), jint(argjson, "winningAmount"));
  argv[5] = NULL;
  argc = 5;
  ...
  if (jint(invoice, "code") != 0) {
    ...
    cJSON_AddNumberToObject(invoiceInfo, "playerID", jint(argjson, "playerID"));
    cJSON_AddNumberToObject(invoiceInfo, "winningAmount",
                            jint(argjson, "winningAmount"));
    cJSON_AddStringToObject(invoiceInfo, "label", argv[3]);
    cJSON_AddStringToObject(invoiceInfo, "invoice", cJSON_Print(invoice));
...
static struct lws_protocols protocols[] = {
    {"http", lws_callback_http_dummy, 0, 0}, {NULL, NULL, 0, 0} /* terminator */
};
...
int32_t BET_p2p_host_deck_init_info(cJSON *argjson, struct privatebet_info *bet,
...
int32_t BET_p2p_host_init(cJSON *argjson, struct privatebet_info *bet,
                          struct privatebet_vars *vars) {
  int32_t peerid, retval = 1;
  bits256 cardpubvalues[CARDS777_MAXCARDS];
  cJSON *cardinfo = NULL;
...
int32_t BET_p2p_bvv_join(cJSON *argjson, struct privatebet_info *bet,
                         struct privatebet_vars *vars) {
...
int32_t BET_p2p_client_join_req(cJSON *argjson, struct privatebet_info *bet,
                                struct privatebet_vars *vars) {
...
int32_t BET_send_turn_info(struct privatebet_info *bet, int32_t playerid,
                           int32_t cardid, int32_t card_type) {
  cJSON *turninfo = NULL;
  int retval = 1, bytes;
  char *rendered = NULL;
...
int32_t BET_p2p_dcv_turn(cJSON *argjson, struct privatebet_info *bet,
                         struct privatebet_vars *vars) {
  int32_t retval = 1;
  ...
  if (hole_cards_drawn == 0) {
    for (int i = 0; i < no_of_hole_cards; i++) {
      for (int j = 0; j < bet->maxplayers; j++) {
        if (card_matrix[j][i] == 0) {
          retval =
              BET_send_turn_info(bet, j, (i * bet->maxplayers) + j, hole_card);
          goto end;
        }
      }
    }
...
int32_t BET_p2p_highest_card(cJSON *argjson, struct privatebet_info *bet,
                             struct privatebet_vars *vars) {
  for (int i = 0; i < 1; i++) {
...
int32_t BET_create_invoice(cJSON *argjson, struct privatebet_info *bet,
                           struct privatebet_vars *vars) {
  ...
  sprintf(argv[3], "%s_%d_%d_%d_%d", bits256_str(hexstr, dcv_info.deckid),
          invoiceID, jint(argjson, "playerID"), jint(argjson, "round"),
          jint(argjson, "betAmount"));
  sprintf(argv[4], "\"Invoice_details_playerID:%d,round:%d,betting Amount:%d\"",
          jint(argjson, "playerID"), jint(argjson, "round"),
          jint(argjson, "betAmount"));
  argv[5] = NULL;
  argc = 5;
...
int32_t BET_create_betting_invoice(cJSON *argjson, struct privatebet_info *bet,
                                   struct privatebet_vars *vars) {
  int argc, bytes, retval = 1;
  char **argv = NULL, *rendered = NULL;
  char hexstr[65];
  ...
  strcpy(argv[1], "invoice");
  sprintf(argv[2], "%ld",
          (long int)jint(argjson, "invoice_amount") * mchips_msatoshichips);
  sprintf(argv[3], "%s_%d_%d_%d_%d", bits256_str(hexstr, dcv_info.deckid),
          invoiceID, jint(argjson, "playerID"), jint(argjson, "round"),
  ...
  if (jint(invoice, "code") != 0)
    ...
    cJSON_AddItemToObject(invoiceInfo, "actionResponse",
                          cJSON_GetObjectItem(argjson, "actionResponse"));
...
int32_t BET_settle_game(cJSON *payInfo, struct privatebet_info *bet,
                        struct privatebet_vars *vars) {
  int32_t playerid, max = -1, retval = 1;
  cJSON *invoicesInfo = NULL, *invoiceInfo = NULL, *invoice = NULL,
        *winnerInfo = NULL;
  char *label = NULL;
  int32_t argc, bytes;
  char **argv = NULL, *rendered = NULL;
...
int32_t BET_p2p_check_player_ready(cJSON *playerReady,
...
int32_t BET_evaluate_hand_test(cJSON *playerCardInfo,
                               struct privatebet_info *bet,
                               struct privatebet_vars *vars) {
    ...
    if (fold_flag == 1) {
      ...
      if (card_values[0][j] != -1)
        cJSON_AddItemToArray(boardCardInfo,
                             cJSON_CreateString(cards[card_values[0][j]]));
      else
        cJSON_AddItemToArray(boardCardInfo, cJSON_CreateNull());
    }
  }
...
void BET_establish_ln_channels(struct privatebet_info *bet) {
  ...
  for (int i = 0; i < bet->maxplayers; i++) {
    ...
    make_command(argc, argv, &connectInfo);
    // ln_bet(argc,argv,buf);
    printf("\n%s:%d:Conncet Response:%s", __FUNCTION__, __LINE__,
           cJSON_Print(connectInfo));
  }
  ...
  for (int i = 0; i < BET_dcv->maxplayers; i++) {
    ...
    while ((channel_state = LN_get_channel_status(channel_id)) != 3) {
      if (channel_state == 2) {
        ...
        retval = -1;
        printf("\n%s:%d: Player: %d is failed to establish the channel with "
...
int32_t BET_award_winner(cJSON *argjson, struct privatebet_info *bet,
                         struct privatebet_vars *vars) {
  int argc, retval = 1;
