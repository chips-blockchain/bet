    ...
    while (1) {
        ...
        if (bytes > 0) {
            ...
            if (0 == strcmp(cJSON_str(cJSON_GetObjectItem(shareInfo, "method")),
                ...
                break;
            } else if (0 == strcmp(cJSON_str(cJSON_GetObjectItem(shareInfo,
                                                                 "method")),
                                   "request_share")) {
        ...
        for (int i = 0; i < bet->range; i++) {
            dcvblindcards[playerID][i] =
                jbits256i(cjsondcvblindcards,
                          playerID * bet->range + i); // bvv_info.maxplayers
        }
    }
    ...
    for (uint32_t i = 0; i < bvv_info.numplayers; i++) {
        for (int j = 0; j < bet->range; j++) {
            cJSON_AddItemToArray(
                cjsonbvvblindcards,
                cJSON_CreateString(bits256_str(str, bvvblindcards[i][j])));
        }
    }
...
bits256 BET_p2p_decode_card(cJSON *argjson, struct privatebet_info *bet,
    ...
    for (int i = 0; i < bet->range; i++) {
        for (int j = 0; j < bet->range; j++) {
            if (bits256_cmp(v_hash[i][j], g_hash[bet->myplayerid][cardid]) ==
                0) {
                for (int m = 0; m < bet->range; m++) {
                    for (int n = 0; n < bet->range; n++) {
                        tmp = curve25519(
                            player_info.player_key.priv,
                            curve25519(
                                player_info.cardprivkeys[m],
                                player_info.cardprods[bet->myplayerid][n]));
                        xoverz = xoverz_donna(tmp);
                        vcalc_sha256(0, hash.bytes, xoverz.bytes,
                                     sizeof(xoverz));
...
int32_t BET_p2p_client_give_share(cJSON *argjson, struct privatebet_info *bet,
    ...
    if ((ptr = BET_decrypt(decipher, sizeof(decipher), player_info.bvvpubkey,
                           player_info.player_key.priv, temp.bytes,
                           &recvlen)) == 0) {
        retval = -1;
        printf("decrypt error \n");
        goto end;
...
int32_t BET_check_player_stack(char *uri) {
    ...
    if (balance >= table_stack) {
        ...
        printf(
            "%s::%d::Insufficient Funds, Minimum needed::%d mCHIPS but only %d "
    ...
    if ((method = jstr(argjson, "method")) != 0) {
        ...
        if (strcmp(method, "join") == 0) {
            ...
            if (jint(argjson, "playerid") == bet->myplayerid) {
                ...
                cJSON_AddNumberToObject(signedTxInfo, "playerid",
                                        bet->myplayerid);
...
int32_t BET_rest_player_init(struct lws *wsi, cJSON *argjson) {
    ...
    for (int i = 0; i < BET_player[playerID]->range; i++) {
        cJSON_AddItemToArray(
            cjsonplayercards,
            cJSON_CreateString(
                bits256_str(str, all_players_info[playerID].cardpubkeys[i])));
    }
...
int32_t BET_rest_player_ready(struct lws *wsi, int32_t playerID) {
    ...
    lws_write(wsi, cJSON_Print(playerReady), strlen(cJSON_Print(playerReady)),
              0);
...

{
    ...
    all_g_shares[playerID] = (struct enc_share *)malloc(
        CARDS777_MAXPLAYERS * CARDS777_MAXPLAYERS * CARDS777_MAXCARDS *
        sizeof(struct enc_share));
    cjsonbvvblindcards = cJSON_GetObjectItem(argjson, "bvvblindcards");
    ...
    for (int i = 0; i < BET_player[playerID]->numplayers; i++) {
        for (int j = 0; j < BET_player[playerID]->range; j++) {
            all_players_info[playerID].bvvblindcards[i][j] = jbits256i(
                cjsonbvvblindcards, i * BET_player[playerID]->range + j);
        }
    }
...
int32_t BET_rest_player_ask_share(struct lws *wsi, int32_t cardid,
    ...
    lws_write(wsi, cJSON_Print(requestInfo), strlen(cJSON_Print(requestInfo)),
              0);
...
int32_t BET_rest_player_give_share(struct lws *wsi, cJSON *argjson) {
    ...
    if ((ptr = BET_decrypt(decipher, sizeof(decipher),
        ...
        lws_write(wsi, cJSON_Print(share_info), strlen(cJSON_Print(share_info)),
                  0);
    }
end:
    return retval;
    ...
    for (int i = 0; i < BET_player[this_playerID]->range; i++) {
        for (int j = 0; j < BET_player[this_playerID]->range; j++) {
            if (bits256_cmp(
                    ...
                    all_g_hash[this_playerID][BET_player[this_playerID]
                                                  ->myplayerid][cardid]) == 0) {
                    ...
                    for (int n = 0; n < BET_player[this_playerID]->range; n++) {
                        tmp = curve25519(
                            ...
                            curve25519(
                                ...
                                all_players_info[this_playerID]
                                    .cardprods[BET_player[this_playerID]
                                                   ->myplayerid][n]));
                        xoverz = xoverz_donna(tmp);
                        vcalc_sha256(0, hash.bytes, xoverz.bytes,
                                     sizeof(xoverz));
                        ...
                        decoded = curve25519(fmul_donna(refval, fe), basepoint);
                        for (int k = 0; k < BET_player[this_playerID]->range;
                             k++) {
                            if (bits256_cmp(
                                    decoded,
                                    all_players_info[this_playerID]
                                        .cardprods[BET_player[this_playerID]
                                                       ->myplayerid][k]) == 0) {
                                all_player_cards[this_playerID]
                                                [no_of_player_cards] = atoi(
                                                    bits256_str(str, decoded));
    ...
    if (all_no_of_shares[this_playerID] ==
        ...
        if (bits256_nonz(decoded256) == 0)
            ...
            for (int k = 0; k < BET_player[this_playerID]->range; k++) {
                if (all_players_info[this_playerID].permis[k] ==
                    decoded256.bytes[30]) {
                    unpermi = k;
                    break;
                }
...
void rest_push_cards(struct lws *wsi, cJSON *argjson, int32_t this_playerID) {
    ...
    for (int32_t i = 0; ((i < no_of_hole_cards) &&
         ...
         i++) {
        cJSON_AddItemToArray(
            holeCardInfo, cJSON_CreateString(
...
void rest_display_cards(cJSON *argjson, int32_t this_playerID) {
    ...
    for (int i = 0; ((i <= jint(argjson, "round")) && (flag)); i++) {
        ...
        for (int j = 0; ((j < BET_player[this_playerID]->maxplayers) && (flag));
             j++) {
            if (jinti(actions,
                      ((i * BET_player[this_playerID]->maxplayers) + j)) > 0)
                printf("\nplayed id:%d, action: %s", j,
                       action_str[jinti(
                           actions,
                           ((i * BET_player[this_playerID]->maxplayers) + j))]);
            count++;
    ...
    if (playerID == BET_player[jint(argjson, "gui_playerID")]->myplayerid) {
        ...
        if (retval) {
            ...
            if (strcmp(jstr(paymentParams, "action"), "small_blind") == 0) {
                retval = BET_rest_small_blind_update(
                    wsi, paymentParams, jint(argjson, "betAmount"));
            } else if (strcmp(jstr(paymentParams, "action"), "big_blind") ==
                       0) {
                retval = BET_rest_big_blind_update(wsi, paymentParams,
                                                   jint(argjson, "betAmount"));
            } else if (strcmp(jstr(paymentParams, "action"), "round_betting") ==
                       0) {
                printf("%s:%d::%s\n", __FUNCTION__, __LINE__,
                       cJSON_Print(argjson));
