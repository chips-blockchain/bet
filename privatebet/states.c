                ...
                for (int j = 0; j < bet->maxplayers; j++) {
                    if ((i != j) &&
                        ...
                        return i;
                           ...
                           vars->bet_actions[i][vars->round] == small_blind) ||
        ...
        for (int j = 0; j < bet->maxplayers; j++) {
            if (vars->bet_actions[j][i] > 0)
                cJSON_AddItemToArray(
                    actions, cJSON_CreateNumber(vars->bet_actions[j][i]));
        }
    }
            ...
            for (int i = 0; i < bet->maxplayers; i++) {
                for (int j = vars->round; j < CARDS777_MAXROUNDS; j++) {
                    vars->bet_actions[i][j] =
                        vars->bet_actions[i][round]; // check
                }
            }
        }
    }
    ...
    for (int i = ((DCV_VARS->turni + 1) % dcv_maxplayers);
        ...
        if ((DCV_VARS->bet_actions[i][DCV_VARS->round] != fold) &&
            ...
            if ((DCV_VARS->bet_actions[i][DCV_VARS->round] == 0) ||
                (DCV_VARS->bet_actions[i][DCV_VARS->round] == small_blind) ||
                (DCV_VARS->bet_actions[i][DCV_VARS->round] == big_blind) ||
                (((DCV_VARS->bet_actions[i][DCV_VARS->round] == check) ||
                  (DCV_VARS->bet_actions[i][DCV_VARS->round] == call) ||
                  (DCV_VARS->bet_actions[i][DCV_VARS->round] == raise)) &&
                 (maxamount != DCV_VARS->betamount[i][DCV_VARS->round]))) {
                retval = i;
                break;
            }
...
int32_t BET_rest_DCV_round_betting(struct lws *wsi) {
    ...
    for (int i = 0; i <= DCV_VARS->round; i++) {
        for (int j = 0; j < dcv_maxplayers; j++) {
            if (DCV_VARS->bet_actions[j][i] > 0)
                cJSON_AddItemToArray(
                    actions, cJSON_CreateNumber(DCV_VARS->bet_actions[j][i]));
        }
    }
    ...
    cJSON_AddNumberToObject(
        roundBetting, "min_amount",
        (maxamount - DCV_VARS->betamount[DCV_VARS->turni][DCV_VARS->round]));

    lws_write(wsi, cJSON_Print(roundBetting), strlen(cJSON_Print(roundBetting)),
              0);
...
int32_t BET_rest_big_blind_update(struct lws *wsi, cJSON *argjson,
    ...
    Player_VARS[this_playerID]->betamount[BET_player[this_playerID]->myplayerid]
...
int32_t BET_rest_player_round_betting(struct lws *wsi, cJSON *argjson) {
    ...
    if (jinti(possibilities, (option - 1)) == raise) {
        ...
        BET_rest_player_create_invoice_request_round(wsi, argjson, bet_amount,
                                                     option);
    } else if (jinti(possibilities, (option - 1)) == call) {
            ...
            for (int i = 0; i < dcv_maxplayers; i++) {
                for (int j = DCV_VARS->round; j < CARDS777_MAXROUNDS; j++) {
                    DCV_VARS->bet_actions[i][j] =
                        DCV_VARS->bet_actions[i][round]; // check
                }
            }
        }
    }
