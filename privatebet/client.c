...
bits256 BET_request_share(int32_t ofCardID, int32_t ofPlayerID,
                          struct privatebet_info *bet, bits256 bvv_public_key,
  ...
  while (1) {
    ...
    if (bytes > 0) {
      ...
      if (0 == strcmp(cJSON_str(cJSON_GetObjectItem(shareInfo, "method")),
                      "response_share")) {
        share = jbits256(shareInfo, "share");
        break;
      } else if (0 ==
                 strcmp(cJSON_str(cJSON_GetObjectItem(shareInfo, "method")),
                        "request_share")) {
...
void BET_give_share(cJSON *shareInfo, struct privatebet_info *bet,
                    bits256 bvv_public_key, struct pair256 player_key) {
  ...
  if ((ofPlayerID == bet->myplayerid) && (forPlayerID != bet->myplayerid)) {
    temp = g_shares[ofPlayerID * bet->numplayers * bet->range +
                    (ofCardID * bet->numplayers + forPlayerID)];
    recvlen = sizeof(temp);
    if ((ptr = BET_decrypt(decipher, sizeof(decipher), bvv_public_key,
                           player_key.priv, temp.bytes, &recvlen)) == 0)
...
struct enc_share get_API_enc_share(cJSON *obj) {
  ...
  if (obj != 0) {
    if (is_cJSON_String(obj) != 0 && (str = obj->valuestring) != 0 &&
        strlen(str) == 176) {
...
int32_t BET_p2p_bvv_init(cJSON *argjson, struct privatebet_info *bet,
  ...
  for (uint32_t playerID = 0; playerID < bvv_info.maxplayers; playerID++) {
    ...
    for (int i = 0; i < bet->range; i++) {
      dcvblindcards[playerID][i] = jbits256i(
          cjsondcvblindcards, playerID * bet->range + i); // bvv_info.maxplayers
    }
  }
  g_shares =
      (struct enc_share *)malloc(CARDS777_MAXPLAYERS * CARDS777_MAXPLAYERS *
                                 CARDS777_MAXCARDS * sizeof(struct enc_share));

  for (uint32_t playerid = 0; playerid < bvv_info.maxplayers; playerid++) {
    p2p_bvv_init(peerpubkeys, bvv_info.bvv_key, bvvblindingvalues[playerid],
                 bvvblindcards[playerid], dcvblindcards[playerid], bet->range,
                 bvv_info.numplayers, playerid, bvv_info.deckid);
  }
  ...
  for (uint32_t i = 0; i < bvv_info.numplayers; i++) {
    for (int j = 0; j < bet->range; j++) {
      cJSON_AddItemToArray(cjsonbvvblindcards, cJSON_CreateString(bits256_str(
                                                   str, bvvblindcards[i][j])));
    }
  }
  ...
  for (int i = 0; i < cJSON_GetArraySize(uriInfo); i++) {
    ...
    while ((channel_state = LN_get_channel_status(channel_id)) != 3) {
      if (channel_state == 2) {
        ...
        retval = -1;
        printf(
            "\n%s:%d: BVV is failed to establish the channel with Player: %d",
...
bits256 BET_p2p_decode_card(cJSON *argjson, struct privatebet_info *bet,
          ...
          for (int n = 0; n < bet->range; n++) {
            ...
            for (int k = 0; k < bet->range; k++) {
              if (bits256_cmp(decoded,
                              player_info.cardprods[bet->myplayerid][k]) == 0) {
...
int32_t BET_p2p_invoice(cJSON *argjson, struct privatebet_info *bet,
                        struct privatebet_vars *vars) {
...
void display_cards(cJSON *argjson, struct privatebet_info *bet,
  ...
  char *cards[52] = {"2C", "3C", "4C",  "5C",  "6C", "7C", "8C", "9C",  "10C",
                     "JC", "QC", "KC",  "AC",  "2D", "3D", "4D", "5D",  "6D",
                     "7D", "8D", "9D",  "10D", "JD", "QD", "KD", "AD",  "2H",
                     "3H", "4H", "5H",  "6H",  "7H", "8H", "9H", "10H", "JH",
                     "QH", "KH", "AH",  "2S",  "3S", "4S", "5S", "6S",  "7S",
  ...
  for (int32_t i = 0; ((i < no_of_hole_cards) && (i < number_cards_drawn));
       i++) {
...
int32_t BET_p2p_client_receive_share(cJSON *argjson,
...
int32_t BET_p2p_client_give_share(cJSON *argjson, struct privatebet_info *bet,
                                  struct privatebet_vars *vars) {
  ...
  if ((ptr = BET_decrypt(decipher, sizeof(decipher), player_info.bvvpubkey,
                         player_info.player_key.priv, temp.bytes, &recvlen)) ==
      0) {
    retval = -1;
    printf("decrypt error \n");
    goto end;
...
int32_t BET_p2p_get_own_share(cJSON *argjson, struct privatebet_info *bet,
                              struct privatebet_vars *vars) {
  struct enc_share temp;
...
int32_t BET_p2p_client_turn(cJSON *argjson, struct privatebet_info *bet,
                            struct privatebet_vars *vars) {
  ...
  if (playerid == bet->myplayerid) {
    ...
    for (int i = 0; i < bet->numplayers; i++) {
      if ((!sharesflag[jint(argjson, "cardid")][i]) && (i != bet->myplayerid)) {
        retval = BET_p2p_client_ask_share(bet, jint(argjson, "cardid"),
...
int32_t BET_p2p_client_player_ready(cJSON *argjson, struct privatebet_info *bet,
                                    struct privatebet_vars *vars) {
...
int32_t BET_p2p_client_bvv_init(cJSON *argjson, struct privatebet_info *bet,
                                struct privatebet_vars *vars)

{
  ...
  for (int i = 0; i < bet->numplayers; i++) {
    for (int j = 0; j < bet->range; j++) {
      player_info.bvvblindcards[i][j] =
          jbits256i(cjsonbvvblindcards, i * bet->range + j);
    }
  }
  ...
  for (int playerid = 0; playerid < bet->numplayers; playerid++) {
    for (int i = 0; i < bet->range; i++) {
      for (int j = 0; j < bet->numplayers; j++) {
        g_shares[k] =
            get_API_enc_share(cJSON_GetArrayItem(cjsonshamirshards, k));
        k++;
      }
    }
  }
...
int32_t BET_p2p_client_dcv_init(cJSON *dcv_info, struct privatebet_info *bet,
                                struct privatebet_vars *vars) {
  int32_t retval = 1;
...
int32_t BET_p2p_client_init(cJSON *argjson, struct privatebet_info *bet,
                            struct privatebet_vars *vars) {
  int32_t bytes, retval = 1;
...
int32_t BET_p2p_client_join_res(cJSON *argjson, struct privatebet_info *bet,
                                struct privatebet_vars *vars) {
  ...
  if (0 ==
      bits256_cmp(player_info.player_key.prod, jbits256(argjson, "pubkey"))) {
         ...
         3)) // 3 means channel is already established with the peer
    {
      ...
      if (jint(fundChannelInfo, "code") != 0) {
        ...
        printf("%s::%d::Existing balance::%d, Needed::%d\n", __FUNCTION__,
               __LINE__, BET_check_player_stack(jstr(argjson, "uri")),
    ...
    if (vars->player_funds == 0) // refill the player stack only if it becomes
                                 // zero and funds are available
...
cJSON *BET_rest_client_join(cJSON *argjson) {
  ...
  key = deckgen_player(player_info.cardprivkeys, player_info.cardpubkeys,
                       player_info.permis, 2);
...
int32_t BET_p2p_client_join(cJSON *argjson, struct privatebet_info *bet,
                            struct privatebet_vars *vars) {
...
void BET_p2p_table_info(cJSON *argjson, struct privatebet_info *bet,
                        struct privatebet_vars *vars) {
...
int32_t BET_rest_player_reset(struct lws *wsi, cJSON *argjson) {
  ...
  Player_VARS[this_PlayerID]->player_funds =
      10000000; // hardcoded to 10000 satoshis
  ...
  memset(BET_player, 0, sizeof *BET_player); //(CARDS777_MAXPLAYERS *sizeof
...
int32_t
BET_player_frontend(struct lws *wsi,
                    cJSON *argjson) // update game state based on host broadcast
{
  ...
  if ((method = jstr(argjson, "method")) != 0) {
    ...
    if (strcmp(method, "player_join") == 0) {
      ...
      retval = BET_player_round_betting_test(argjson, BET_player_global,
                                             Player_VARS_global);
    } else if (strcmp(method, "reset") == 0) {
...
int lws_callback_http_player(struct lws *wsi, enum lws_callback_reasons reason,
  ...
  case LWS_CALLBACK_RECEIVE:
    ...
    if (BET_player_frontend(wsi, argjson) != 1) {
      printf("\n%s:%d:Failed to process the host command", __FUNCTION__,
             __LINE__);
    }
...
void player_sigint_handler(int sig) { interrupted1 = 1; }
...
static struct lws_protocols protocols_bvv[] = {
    {"http", lws_callback_http_bvv, 0, 0}, {NULL, NULL, 0, 0} /* terminator */
};
...
void BET_push_client(cJSON *argjson) { player_lws_write(argjson); }
...
int32_t BET_player_backend(
    cJSON *argjson, struct privatebet_info *bet,
    struct privatebet_vars *vars) // update game state based on host broadcast
{
  ...
  if ((method = jstr(argjson, "method")) != 0) {
    ...
    if (strcmp(method, "join") == 0) {
      ...
      retval = BET_player_create_invoice(
          argjson, bet, vars, bits256_str(hexstr, player_info.deckid));
    } else if (strcmp(method, "reset") == 0) {
      retval = BET_player_reset(bet, vars);
    } else if (strcmp(method, "seats") == 0) {
    ...
    if (bet->subsock >= 0 && bet->pushsock >= 0) {
      ...
      if ((recvlen > 0) && ((msgjson = cJSON_Parse(tmp)) != 0)) {
        if (BET_player_backend(msgjson, bet, Player_VARS_global) < 0) {
          printf("\nFAILURE\n");
          // do something here, possibly this could be because unknown commnad
          // or because of encountering a special case which state machine fails
          // to handle
        }
        if (tmp)
          free(tmp);
...
int32_t BET_rest_player(struct lws *wsi, cJSON *argjson) {
  ...
  player_info.player_key.priv =
      curve25519_keypair(&player_info.player_key.prod);
...
int32_t BET_rest_player_join(struct lws *wsi, cJSON *argjson) {
  ...
  BET_player[player_id]->maxplayers =
      (Maxplayers < CARDS777_MAXPLAYERS) ? Maxplayers : CARDS777_MAXPLAYERS;
  ...
  key = deckgen_player(all_players_info[player_id].cardprivkeys,
                       all_players_info[player_id].cardpubkeys,
                       all_players_info[player_id].permis,
                       BET_player[player_id]->range);
  ...
  for (int i = 0; i < BET_player[playerID]->range; i++) {
    cJSON_AddItemToArray(cjsonplayercards,
                         cJSON_CreateString(bits256_str(
                             str, all_players_info[playerID].cardpubkeys[i])));
  }
...
int32_t BET_rest_player_process_init_d(struct lws *wsi, cJSON *argjson,
  ...
  for (int i = 0; i < BET_player[playerID]->numplayers; i++) {
    for (int j = 0; j < BET_player[playerID]->range; j++) {
      all_players_info[playerID].cardprods[i][j] =
          jbits256i(cjsoncardprods, i * BET_player[playerID]->range + j);
    }
  }
  ...
  for (int i = 0; i < BET_player[playerID]->numplayers; i++) {
    for (int j = 0; j < BET_player[playerID]->range; j++) {
      all_g_hash[playerID][i][j] =
          jbits256i(cjsong_hash, i * BET_player[playerID]->range + j);
    }
  }
...
int32_t BET_rest_bvv_compute_init_b(struct lws *wsi, cJSON *argjson) {
    ...
    for (int i = 0; i < BET_bvv->range; i++) {
      dcvblindcards[playerID][i] =
          jbits256i(cjsondcvblindcards,
                    playerID * BET_bvv->range + i); // bvv_info.maxplayers
    }
  }

  g_shares =
      (struct enc_share *)malloc(BET_bvv->maxplayers * BET_bvv->maxplayers *
                                 BET_bvv->range * sizeof(struct enc_share));

  for (uint32_t playerid = 0; playerid < bvv_info.maxplayers; playerid++) {
    p2p_bvv_init(peerpubkeys, bvv_info.bvv_key, bvvblindingvalues[playerid],
                 bvvblindcards[playerid], dcvblindcards[playerid],
                 BET_bvv->range, bvv_info.numplayers, playerid,
                 bvv_info.deckid);
    // sleep(5);
  }
  ...
  strncpy(temp_buf + LWS_PRE, cJSON_Print(bvv_init_info),
...

{
  ...
  for (int i = 0; i < BET_player[playerID]->numplayers; i++) {
    for (int j = 0; j < BET_player[playerID]->range; j++) {
      all_players_info[playerID].bvvblindcards[i][j] =
          jbits256i(cjsonbvvblindcards, i * BET_player[playerID]->range + j);
    }
  }

  cjsonshamirshards = cJSON_GetObjectItem(argjson, "shamirshards");
  int k = 0;
  for (int playerid = 0; playerid < BET_player[playerID]->numplayers;
       playerid++) {
    for (int i = 0; i < BET_player[playerID]->range; i++) {
      for (int j = 0; j < BET_player[playerID]->numplayers; j++) {
        all_g_shares[playerID][k] =
            get_API_enc_share(cJSON_GetArrayItem(cjsonshamirshards, k));
        k++;
      }
    }
  ...
  for (int i = 0; i < BET_player[playerID]->range; i++) {
    for (int j = 0; j < BET_player[playerID]->range; j++) {
      temp = xoverz_donna(curve25519(
          all_players_info[playerID].player_key.priv,
          curve25519(playerprivs[i],
                     all_players_info[playerID]
                         .cardprods[BET_player[playerID]->myplayerid][j])));
      vcalc_sha256(0, all_v_hash[playerID][i][j].bytes, temp.bytes,
                   sizeof(temp));
    }
  }
...
int32_t BET_rest_fundChannel(char *channel_id) {
  ...
  while ((channel_state = LN_get_channel_status(channel_id)) != 3) {
    if (channel_state == 2) {
      ...
      retval = -1;
      printf("\n%s:%d:Channel establishment with DCV is not happening, please "
...
int32_t BET_rest_player_join_res(cJSON *argjson) {
  ...
  if (0 ==
      bits256_cmp(all_players_info[jint(argjson, "peerid")].player_key.prod,
...
int32_t BET_rest_player_turn(struct lws *wsi, cJSON *argjson) {
  ...
  if (playerid == BET_player[this_playerID]->myplayerid) {
    ...
    for (int i = 0; i < BET_player[this_playerID]->numplayers; i++) {
      if ((!all_sharesflag[this_playerID][jint(argjson, "cardid")][i]) &&
          (i != BET_player[this_playerID]->myplayerid)) {
        retval = BET_rest_player_ask_share(
            wsi, jint(argjson, "cardid"), jint(argjson, "playerid"),
...
int32_t BET_rest_player_give_share(struct lws *wsi, cJSON *argjson) {
  ...
  temp = all_g_shares[this_playerID]
                     [playerid * BET_player[this_playerID]->numplayers *
                          BET_player[this_playerID]->range +
  ...
  if ((ptr = BET_decrypt(decipher, sizeof(decipher),
    ...
    cJSON_AddNumberToObject(share_info, "cardid", cardid);
    cJSON_AddNumberToObject(share_info, "card_type", card_type);
    cJSON_AddNumberToObject(share_info, "toPlayer",
                            jint(argjson, "fromPlayer"));
    cJSON_AddNumberToObject(share_info, "fromPlayer",
                            jint(argjson, "toPlayer"));
    jaddbits256(share_info, "share", share);
...
bits256 BET_rest_decode_card(int32_t this_playerID, int32_t cardid) {
  ...
  M = (numplayers / 2) + 1;
  for (int i = 0; i < M; i++) {
    memcpy(shares[i], all_playershares[this_playerID][cardid][i].bytes,
           sizeof(bits256));
  }
  gfshare_calc_sharenrs(
      sharenrs, numplayers, all_players_info[this_playerID].deckid.bytes,
  ...
  refval = fmul_donna(
      all_players_info[this_playerID]
          .bvvblindcards[BET_player[this_playerID]->myplayerid][cardid],
      crecip_donna(recover));
  ...
  for (int i = 0; i < BET_player[this_playerID]->range; i++) {
    for (int j = 0; j < BET_player[this_playerID]->range; j++) {
      bits256 temp = xoverz_donna(curve25519(
          all_players_info[this_playerID].player_key.priv,
          curve25519(
              all_players_info[this_playerID].cardprivkeys[i],
              all_players_info[this_playerID]
                  .cardprods[BET_player[this_playerID]->myplayerid][j])));
      vcalc_sha256(0, all_v_hash[this_playerID][i][j].bytes, temp.bytes,
                   sizeof(temp));
    }
  }
  ...
  for (int i = 0; i < BET_player[this_playerID]->range; i++) {
    for (int j = 0; j < BET_player[this_playerID]->range; j++) {
      if (bits256_cmp(
              ...
              all_g_hash[this_playerID][BET_player[this_playerID]->myplayerid]
                        [cardid]) == 0) {
        for (int m = 0; m < BET_player[this_playerID]->range; m++) {
          for (int n = 0; n < BET_player[this_playerID]->range; n++) {
            tmp = curve25519(
                all_players_info[this_playerID].player_key.priv,
                curve25519(
                    all_players_info[this_playerID].cardprivkeys[m],
                    all_players_info[this_playerID]
                        .cardprods[BET_player[this_playerID]->myplayerid][n]));
            xoverz = xoverz_donna(tmp);
            vcalc_sha256(0, hash.bytes, xoverz.bytes, sizeof(xoverz));
            ...
            for (int k = 0; k < BET_player[this_playerID]->range; k++) {
              if (bits256_cmp(
                      decoded,
                      all_players_info[this_playerID]
                          .cardprods[BET_player[this_playerID]->myplayerid]
                                    [k]) == 0) {
                all_player_cards[this_playerID][no_of_player_cards] =
                    atoi(bits256_str(str, decoded));
...
int32_t BET_rest_player_receive_share(struct lws *wsi, cJSON *argjson) {
  int32_t retval = 1, cardid, playerid, errs = 0, unpermi, card_type,
          this_playerID;
  cJSON *playerCardInfo = NULL;
  bits256 share, decoded256;
  ...
  if (all_no_of_shares[this_playerID] ==
      BET_player[this_playerID]->maxplayers) {
    all_no_of_shares[this_playerID] = 0;
    ...
    if (unpermi != -1) {
      all_player_card_values[this_playerID]
                            [all_number_cards_drawn[this_playerID]++] =
                                decoded256.bytes[30];
      playerCardInfo = cJSON_CreateObject();
      cJSON_AddStringToObject(playerCardInfo, "method", "playerCardInfo");
      ...
      cJSON_AddNumberToObject(playerCardInfo, "decoded_card",
                              decoded256.bytes[30]);
      lws_write(wsi, cJSON_Print(playerCardInfo),
                strlen(cJSON_Print(playerCardInfo)), 0);
    }
  }
  return retval;
}
...
void rest_push_cards(struct lws *wsi, cJSON *argjson, int32_t this_playerID) {
  ...
  holeCardInfo = cJSON_CreateArray();
  for (int32_t i = 0;
       ((i < no_of_hole_cards) && (i < all_number_cards_drawn[this_playerID]));
       i++) {
    cJSON_AddItemToArray(
        holeCardInfo,
  ...
  boardCardInfo = cJSON_CreateArray();
  for (int32_t i = no_of_hole_cards;
       ((i < hand_size) && (i < all_number_cards_drawn[this_playerID])); i++) {
    cJSON_AddItemToArray(
        boardCardInfo,
        cJSON_CreateString(cards[all_player_card_values[this_playerID][i]]));
  }
...
void rest_display_cards(cJSON *argjson, int32_t this_playerID) {
  ...
  char action_str[8][100] = {"",      "small_blind", "big_blind", "check",
  ...
  for (int i = 0; ((i <= jint(argjson, "round")) && (flag)); i++) {
    printf("\nRound:%d", i);
    for (int j = 0; ((j < BET_player[this_playerID]->maxplayers) && (flag));
         j++) {
      if (jinti(actions, ((i * BET_player[this_playerID]->maxplayers) + j)) > 0)
        printf(
            "\nplayed id:%d, action: %s", j,
            action_str[jinti(
                actions, ((i * BET_player[this_playerID]->maxplayers) + j))]);
      count++;
  ...
  if (playerID == BET_player[jint(argjson, "gui_playerID")]->myplayerid) {
    ...
    if (retval) {
      ...
      if (strcmp(jstr(paymentParams, "action"), "small_blind") == 0) {
        ...
        printf("%s:%d::%s\n", __FUNCTION__, __LINE__, cJSON_Print(argjson));
        retval = BET_rest_player_round_betting_update(
            wsi, paymentParams, jint(argjson, "option"),
            jint(argjson, "betAmount"));
      }
    }
  }
