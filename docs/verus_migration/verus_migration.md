...
In bet we are presenting the data in JSON format and exhcanging it over sockets, unlike socket communications in vdxf ID's we need to be veru careful in how we storing the data on chain and what data we storing on the chain. To minimize the storage space on blockchain, we encode the data as compact strucutres and store on the chain and that saves some space in representing the data. 
