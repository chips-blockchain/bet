...
int32_t BET_DCV_next_turn(cJSON *argjson, struct privatebet_info *bet,
  ...
  for (int i = ((vars->turni + 1) % bet->maxplayers); (i != vars->turni);
    ...
    if ((vars->bet_actions[i][vars->round] != fold) &&
        (vars->bet_actions[i][vars->round] != allin) && (vars->funds[i] != 0)) {
      if (vars->bet_actions[i][vars->round] == 0) {

        for (int j = 0; j < bet->maxplayers; j++) {
          if ((i != j) && (((vars->bet_actions[j][vars->round] == 0) &&
                            (vars->funds[j] != 0)) ||
...
int32_t BET_DCV_round_betting(cJSON *argjson, struct privatebet_info *bet,
  ...
  for (int i = 0; i < bet->maxplayers; i++) {
    printf("%s::%d::player id::%d::funds::%d::vars->round::%d\n", __FUNCTION__,
           __LINE__, i, vars->funds[i], vars->round);
    ...
    for (int j = 0; j < bet->maxplayers; j++) {
      if (vars->bet_actions[j][i] > 0)
        cJSON_AddItemToArray(actions,
                             cJSON_CreateNumber(vars->bet_actions[j][i]));
    }
  }
  ...
  if ((action = jstr(argjson, "action")) != 0) {
    ...
    if (strcmp(action, "small_blind") == 0) {
      ...
      for (int i = 0; i < bet->maxplayers; i++) {
        ...
        cJSON_AddItemToArray(player_funds,
...
int32_t BET_player_round_betting_test(cJSON *argjson,
  ...
  if (jinti(possibilities, (option - 1)) == raise) {
    ...
    retval = BET_player_create_betting_invoice_request(argjson, action_response,
                                                       bet, invoice_amount);
    if (retval < 0)
      goto end;
...
int32_t BET_player_round_betting(cJSON *argjson, struct privatebet_info *bet,
  ...
  if (jinti(possibilities, (option - 1)) == raise) {
    ...
    } while ((raise_amount < min_amount) || (raise_amount < big_blind_amount) ||
...
int32_t BET_rest_player_dealer_info(struct lws *wsi, cJSON *argjson,
  ...
  if (Player_VARS[playerID]->dealer == BET_player[playerID]->myplayerid) {
    dealerReady = cJSON_CreateObject();
    cJSON_AddStringToObject(dealerReady, "method", "dealer_ready");
    lws_write(wsi, cJSON_Print(dealerReady), strlen(cJSON_Print(dealerReady)),
              0);
  }
...
int32_t BET_rest_DCV_next_turn(struct lws *wsi) {
  ...
  for (int i = ((DCV_VARS->turni + 1) % dcv_maxplayers); (i != DCV_VARS->turni);
       i = ((i + 1) % dcv_maxplayers)) {
  ...
  for (int i = 0; i <= DCV_VARS->round; i++) {
    for (int j = 0; j < dcv_maxplayers; j++) {
      if (DCV_VARS->bet_actions[j][i] > 0)
        cJSON_AddItemToArray(actions,
                             cJSON_CreateNumber(DCV_VARS->bet_actions[j][i]));
    }
  }
...
int32_t BET_rest_player_small_blind_bet(struct lws *wsi, cJSON *argjson,
  ...
  playerid = jint(argjson, "playerid");
  round = jint(argjson, "round");
  Player_VARS[this_playerID]->turni = (Player_VARS[this_playerID]->turni + 1) %
...
int32_t BET_rest_betting_statemachine(struct lws *wsi, cJSON *argjson) {
  ...
  if ((action = jstr(argjson, "action")) != 0) {
    ...
    if (strcmp(action, "small_blind") == 0) {
      ...
      retval = BET_rest_player_small_blind_bet(wsi, argjson,
