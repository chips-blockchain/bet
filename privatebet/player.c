	...
	for (int32_t i = 0; i < CARDS777_MAXCARDS; i++) {
		for (int32_t j = 0; j < CARDS777_MAXCARDS; j++) {
			if (strcmp(bits256_str(str1, d_blinded_card),
				   bits256_str(str2, curve25519(p_deck_info.player_r[i].priv, jbits256i(dealer_blind_info,j)))) == 0) {
...
void reveal_card(char *table_id)
{
	...
	if((player_id == player_config.player_id) || (player_id == -1)){
		...
		b_blinded_deck = get_cJSON_from_id_key_vdxfid(table_id, get_key_data_vdxf_id(all_t_b_p_keys[player_id+1], game_id_str));
		...
		dealer_blind_info = get_cJSON_from_id_key_vdxfid(table_id, get_key_data_vdxf_id(T_D_DECK_KEY,game_id_str));
